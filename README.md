...
- `UDanzmannExperienceAction` (`UObject`): a lighter `UGameFeatureAction`. Override `OnExperienceActivated()`/`OnExperienceDeactivated()`; each is called once per session by the manager.
